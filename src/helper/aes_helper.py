...
def string_encrypt(clear, min_size):
    curr_size = min_size
    while True:
        output = ""
        for b in SHAKE256(clear.encode("utf-8"), curr_size):
            output += str(hex(b))[2:]
        cipher = str(base64.b64encode(output.encode("utf-8")))[2:-3]
        if cipher.__len__() <= min_size:
            return cipher
        curr_size -= 1
